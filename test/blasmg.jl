    ...
    if CUDA.has_cublasmg()
        ...
        @testset "element type $elty" for elty in [Float32, Float64]
            @testset "mg_gemm!" begin
                ...
                B = rand(elty, k, n)
                d_C = copy(C)
                d_C = CUBLASMG.mg_gemm!('N','N',alpha,A,B,beta,d_C, devs=devs, dev_rows=drs, dev_cols=dcs)
                # compare
                ...
                B = rand(elty, k, n)
                d_C = copy(C)
                d_C = CUBLASMG.mg_gemm!('T','N',alpha,A,B,beta,d_C, devs=devs, dev_rows=drs, dev_cols=dcs)
                # compare
                ...
                B = rand(elty, n, k)
                d_C = copy(C)
                d_C = CUBLASMG.mg_gemm!('N','T',alpha,A,B,beta,d_C, devs=devs, dev_rows=drs, dev_cols=dcs)
                # compare
